...
def visit_eqt_node_html(self, node):
    """
...
def depart_eqt_node_html(self, node):
    """
...
def visit_depart_do_nothing(self, node):
    pass
    ...
    def run(self):
        ...
        if self.name == "eqt" or self.name == "eqt-mc":
            ...
            if answer_list_node is not None:
                #> Add the eqt-answer-list class
                answer_list_node.set_class('eqt-answer-list')
            else:
...
def eqt_answer(name,
    ...
    #     return [
    #         EqtAnswerType(args=options, content=text, p_to_static=p_to_static)
    #     ], []
...
def visit_explanation_node(self, node):
    ...
    """
    self.body.append('<div class="admonition tip expl" style="display: none;">'
                     '%s</div>' % node["content"])
    return
...
def setup(app):
    app.add_node(Eqt,
                 html=(visit_eqt_node_html,
                       depart_eqt_node_html),
                 latex=(visit_depart_do_nothing,
                        visit_depart_do_nothing)
                )
